 ...
 *    files the node under `incomplete`. The chart cards and verdict badges sit
 *    on translucent tints, so their text composites onto something axe will not
 *    measure.
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The sweep and
 *    parameter tables sit in `.table-wrap` scrollers and the verification
 *    transcript is a `<pre>`, so at 380px much of them is clipped away.
 ...
 *    never laid down.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. The error
 *    profile and goodness-of-fit charts draw a shape and then its label as a
 *    following `<text>`, so a label's backdrop is a PRECEDING SIBLING shape,
 *    not an ancestor. An ancestor-only
